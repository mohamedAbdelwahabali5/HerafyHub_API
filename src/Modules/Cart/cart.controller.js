...
exports.removeFromCart = async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ message: "Unauthorized: Please log in to remove items from the cart." });
        }
...
exports.getCart = async (req, res) => {
  try {
      ...
      if (!req.user) {
          return res.status(401).json({ message: "Unauthorized: Please log in to view the cart." });
      }
...
exports.clearCart = async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ message: "Unauthorized: Please log in to clear the cart." });
        }
